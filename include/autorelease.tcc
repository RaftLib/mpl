...
template< class T, 
          class FIFO, 
          mpl::autotype type = poptype > class autorelease : public autoreleasebase
{
public:
   using self_t = autorelease< T, FIFO, poptype >;
   
   /**
    * constructor,only used by the fifo
    * @param fifo - FIFO&, current fifo that the object points to
    */
   constexpr autorelease( const T           &i, 
                          const mpl::signal &s, 
                          FIFO              &f ) : autoreleasebase(),
                                                   item( i ),
                                                   signal( s ),
                                                   fifo( f )
   {
   ...
   /** copy constructor **/
   constexpr autorelease( const self_t  &other ) : 
                                           item( other.item ),
                                           signal( other.signal ),
                                           fifo( other.fifo )
   {
       ...
       */
      const_cast< self_t& >( other ).copied = true;
   }
    ...
    * @return mpl::signal
    */
   const mpl::signal& sig() const noexcept
   {
      return( signal );
   }
   
   friend std::ostream& operator << ( std::ostream &os, const self_t &t )
   {
        os << "item: " << t.item << " - " << mpl::signal_strings[ t.signal ];
        return( os );
   }
...
template < class T, class FIFO > class autorelease< T, FIFO, allocatetype > : 
    public autoreleasebase
{
public:
   /** 
    * this is simply the type of this class 
    */
   using self_t = autorelease< T, FIFO, allocatetype >;

   constexpr autorelease( T &item, FIFO &fifo ) : item( item ), fifo( fifo )
   {
   }
...
#endif /** got rid for the moment while getting up and running **/

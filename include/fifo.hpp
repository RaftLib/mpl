...
public:
          ...
          std::vector< std::reference_wrapper< T > >
    {
       ...
       /** 
        * compiler should optimize this copy, if not then 
        * it'll be a copy of referneces not full objects 
        */
       return( std::forward<
          std::vector< 
     ...
     * @param   signal - const mpl::signal, default: NONE
     */
    virtual void send( const mpl::signal = mpl::none ) = 0;
     ...
     * @param   signal - const mpl::signal, default: NONE
     */
     virtual void send_range( const mpl::signal = mpl::none ) = 0;
     ...
     * @param   item -  T&
     * @param   signal -  mpl::signal, default mpl::none
     */
    template < class T >
    void push( const T &item, const mpl::signal signal = mpl::none )
    {
    ...
    void insert(   iterator_type begin,
                   iterator_type end,
                   const mpl::signal signal = mpl::none )
    {
     ...
     * @param   item - T&
     * @param   signal - mpl::signal
     */
    template< class T >
    void pop( T &item, mpl::signal *signal = nullptr )
    {
     ...
     * @param   items    - std::vector< std::pair< T, mpl::signal > >& 
     ...
     * being used.  
     * @param   signal - mpl::signal, default: nullptr
     * @return T&
     */
    template< class T,
              typename std::enable_if< inline_alloc< T >::value >::type* = nullptr >
    T& peek( mpl::signal *signal = nullptr )
    {
       void *ptr( nullptr );
...
protected:
    ...
    * @param   ptr - void* 
    * @param   signal - mpl::signal reference
    */
   virtual void local_push( void *ptr, const mpl::signal &signal ) = 0;

   /**
    * local_insert - inserts a range from ptr_begin to ptr_end
    * and inserts the signal at the last element inserted, the 
    * rest of the signals are set to mpl::none.
    * @param   ptr_begin - void*
    * @param   ptr_end   - void*
    * @param   signal    - const mpl::signal& 
    */
    ...
    * @param   signal - mpl::signal* 
    */
   virtual void local_pop( void *ptr, mpl::signal *signal ) = 0;
